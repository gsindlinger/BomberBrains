...
def main(argv = None):
    ...
    play_parser.add_argument("--scenario", default="classic")
    ...
    if args.scenario == "strategy" and args.train != 0:
        SINGLE_MODE = True
        if not SINGLE_MODE:
            for j in range(3):
                run_advanced_strategy(agents, args, every_step, gui, j)
        else:
            exploration_strategy = 0
            run_advanced_strategy(agents, args, every_step, gui, exploration_strategy)


    elif args.scenario == "classic-test" and args.train != 0:
        exploration_strategy = 0
        args.scenario = "classic"
        world = BombeRLeWorld(args, agents, exploration_strategy)
...
def run_advanced_strategy(agents, args, every_step, gui, j):
    round_helper = 1000

    for k in range(args.n_rounds // round_helper):
        for i, scenario in enumerate(s.SCENARIOS_TRAIN):
            print(f"Training for Exploration Mode: {j}, Round: {k+1} of {args.n_rounds // round_helper}, "
                  f"Scenario: {scenario}")
            args.scenario = scenario
            world = BombeRLeWorld(args, agents, j)
            world_controller(world, round_helper,
                             gui=gui, every_step=every_step, turn_based=args.turn_based,
                             make_video=args.make_video, update_interval=args.update_interval)
